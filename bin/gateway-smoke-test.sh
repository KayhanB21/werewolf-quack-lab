...
assert_jq "${whoami_json}" 'length == 5' "whoami returns all five Quack nodes"
assert_jq "${whoami_json}" '[.[].name] | sort == ["agent-a","agent-b","agent-c","agent-d","agent-e"]' "whoami returns expected node names"

public_json="$(federate_json "SELECT round, agent_id, public_text FROM public_intents")"
assert_jq "${public_json}" 'length == 5' "public_log federation returns five public rows"
...
assert_jq "${wolf_json}" 'length == 2' "wolf_channel returns only wolf rows"
assert_jq "${wolf_json}" '[.[].agent_id] | sort == ["agent-a","agent-d"]' "wolf_channel row filtering is evaluated on player nodes"
...
set +e
denied_output="$(
  duck_json "SELECT * FROM quack_query('quack:agent-a:9494', 'SELECT round, agent_id, rationale FROM intents', token => '${AGENT_A_TOKEN:-agent-a-dev-token}', disable_ssl => true);" 2>&1
)"
denied_status="$?"
set -e
